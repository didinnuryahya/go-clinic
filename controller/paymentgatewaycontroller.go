...
var PriorityModel []models.PriorityModel

// GetUsers get all the user record form db
func GetPaymentGateways(c *gin.Context) {
	var paymentgateways []models.PaymentGateway
	if err := tx.Find(&paymentgateways).Error; err != nil {
		c.AbortWithStatus(404)
		fmt.Println(err)
	} else {
		c.JSON(200, paymentgateways)
	}
}

// GetUser get single user record form db
func GetPaymentGateway(c *gin.Context) {
	id := c.Params.ByName("id")
	var paymentgateway models.PaymentGateway
	if err := tx.Where("id = ?", id).First(&paymentgateway).Error; err != nil {
		c.AbortWithStatus(404)
		fmt.Println(err)
	} else {
		c.JSON(200, paymentgateway)
	}
}
...
func (priority ArticleModel) isPriorityBy(user ArticleUserModel) bool {
	db := database.GetDB()
	var priority []models.PriorityModel
